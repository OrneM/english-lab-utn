  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8 animate-fade-in">
      ...
      <div className="text-center space-y-3">
        <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 text-emerald-700 dark:text-emerald-400 text-xs font-semibold">
      ...
      {/* Navigation Tabs */}
      <div className="flex items-center justify-center space-x-2 border-b border-slate-200 dark:border-slate-800 pb-4">
        {[
          ...
          <div className="glass-card rounded-2xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 space-y-6 shadow-xl">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 dark:border-slate-800/80 pb-4">
              <div className="space-y-1">
                <div className="flex items-center space-x-2 text-brand-700 dark:text-brand-400 font-bold text-base">
                ...
                <p className="text-slate-700 dark:text-slate-300 text-xs sm:text-sm leading-relaxed max-w-3xl">
                  {data.overview.definition}
                </p>
              </div>
              <span className="self-start sm:self-center px-3 py-1 rounded-full bg-brand-50 dark:bg-brand-500/10 border border-brand-200 dark:border-brand-500/20 text-brand-700 dark:text-brand-400 text-xs font-semibold whitespace-nowrap font-mono">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5">
                  {data.overview.frequencyScale.map((item, idx) => (
                    <div key={idx} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800/90 hover:border-slate-300 dark:hover:border-slate-700 transition-all flex flex-col justify-between space-y-1.5">
                      <div className="flex items-center justify-between">
                      ...
                      <div>
                        <div className="font-bold text-slate-900 dark:text-slate-100 text-xs">{item.adverb}</div>
                        <div className="text-[11px] text-slate-600 dark:text-slate-400">{item.translation}</div>
                      </div>
                      <div className="text-[10px] text-slate-500 italic pt-1 border-t border-slate-200 dark:border-slate-800/60 truncate" title={item.itExample}>
                        "{item.itExample}"
                      </div>
                    </div>
                  ))}
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {data.overview.syntaxRules.map((ruleItem, rIdx) => (
                    <div key={rIdx} className="p-4 sm:p-5 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-3 flex flex-col justify-between hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                      <div className="space-y-2">
                        <div className="flex items-center justify-between gap-2">
                          <h5 className="font-bold text-slate-900 dark:text-slate-100 text-sm">{ruleItem.title}</h5>
                        ...
                        {/* Sintaxis Formula */}
                        <div className="p-2.5 rounded-lg bg-white dark:bg-slate-950/90 border border-slate-200 dark:border-slate-800 font-mono text-[11px] text-cyan-800 dark:text-cyanBrand-300">
                      ...
                      <div className="pt-2 border-t border-slate-200 dark:border-slate-800/80 space-y-1.5">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:text-slate-400 block">Ejemplos IT:</span>
                        {ruleItem.examples.map((ex, exIdx) => (
                          <div key={exIdx} className="text-xs bg-white dark:bg-slate-950/50 p-2 rounded-lg border border-slate-200 dark:border-slate-800/60 space-y-0.5">
                            <div className="font-medium text-slate-900 dark:text-slate-200">"{ex.en}"</div>
                            <div className="text-[11px] text-slate-600 dark:text-slate-400">{ex.es}</div>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <div className="pt-3 border-t border-slate-200 dark:border-slate-800">
              <span className="text-xs font-semibold text-slate-600 dark:text-slate-400 block mb-2">Marcadores de tiempo y expresiones de frecuencia frecuentes:</span>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                            {struct.spellingRules.map((sp, spIdx) => (
                              <div key={spIdx} className="p-3 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-xs space-y-1">
                                ...
                                <div className="text-brand-700 dark:text-brand-300 font-mono font-medium">{sp.examples.join(' · ')}</div>
                              </div>
                            ))}
                          </div>
                        </div>
      ...
      {activeTab === 'vocabulary' && (
        <div className="space-y-8 animate-fade-in">
          ...
          <div className="glass-card rounded-2xl p-6 border border-slate-200 dark:border-slate-800 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {data.vocabularySection.roles.map((role, idx) => (
                <div key={idx} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-1 hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                  <div className="font-bold text-cyan-800 dark:text-cyanBrand-300 text-sm">{role.role}</div>
          ...
          <div className="glass-card rounded-2xl p-6 border border-slate-200 dark:border-slate-800 space-y-4">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center space-x-2">
              <MessageSquare className="w-5 h-5 text-cyan-600 dark:text-cyanBrand-400" />
              <span>Saludos Formales e Informales</span>
            </h3>
            <div className="space-y-2.5">
              {data.vocabularySection.greetings.map((greet, idx) => (
                <div key={idx} className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div className="flex items-center space-x-3">
                    ...
                    <span className="font-bold text-slate-900 dark:text-slate-100 text-sm">"{greet.text}"</span>
                  </div>
                  <span className="text-xs text-slate-600 dark:text-slate-400 sm:text-right">{greet.situation}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="glass-card rounded-2xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 space-y-6 animate-fade-in">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            <button
              ...
              className="text-xs text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 font-semibold underline self-start sm:self-auto"
            >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {q.options.map((opt, optIdx) => {
                      ...
                      if (quizChecked) {
                        ...
                        btnStyle = 'bg-brand-50 dark:bg-brand-600/30 border-brand-500 text-brand-900 dark:text-white font-bold ring-2 ring-brand-500/30';
                      }
