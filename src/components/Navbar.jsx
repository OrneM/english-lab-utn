    ...
    <header className="fixed top-0 left-0 right-0 z-50 glass-nav transition-all duration-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Desktop Navigation */}
          <nav className="hidden md:flex items-center space-x-1 bg-slate-900/60 light:bg-slate-100/80 p-1.5 rounded-2xl border border-slate-800/80 light:border-slate-200">
            {navItems.map((item) => {
                ...
                <button
                  ...
                  className={`relative flex items-center space-x-2 px-3.5 py-2 rounded-xl text-sm font-semibold transition-all duration-200 ${
                    isActive
                      ...
                      : 'text-slate-300 light:text-slate-600 hover:text-white light:hover:text-slate-900 hover:bg-slate-800/50 light:hover:bg-slate-200/50'
                  }`}
                >
          ...
          <div className="flex items-center space-x-2">
            ...
            <button
              ...
              className="p-2 rounded-xl bg-slate-800/50 light:bg-slate-100 text-slate-300 light:text-slate-700 border border-slate-700/50 light:border-slate-300 hover:bg-slate-800 light:hover:bg-slate-200 transition-colors"
            >
            ...
            <button
              ...
              className="md:hidden p-2 rounded-xl bg-slate-800/60 text-slate-300 border border-slate-700/50"
            >
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-slate-800 light:border-slate-200 bg-slate-950/95 light:bg-white/95 px-4 pt-3 pb-5 space-y-1.5 shadow-2xl animate-fade-in">
          {navItems.map((item) => {
            const Icon = item.icon;
            const isActive = activeSection === item.id;
            return (
              ...
              >
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full ${
                  isActive ? 'bg-brand-700 text-cyan-200' : 'bg-slate-800 light:bg-slate-200 text-slate-400 light:text-slate-600'
                }`}>
                  {item.badge}
                </span>
              </button>
