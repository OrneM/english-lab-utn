  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-8 animate-fade-in">
      ...
      <div className="text-center space-y-3">
        <div className="inline-flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 text-amber-800 dark:text-amber-400 text-xs font-semibold">
      ...
      {/* TAB 1: BUSCADOR DE VERBOS IRREGULARES */}
      {activeTab === 'verbs' && (
        <div className="space-y-8 animate-fade-in">
          
          {/* Search Box Header */}
          <div className="glass-card rounded-2xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 space-y-6 shadow-xl">
            <div className="space-y-1 text-center sm:text-left">
              <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-slate-100 flex items-center justify-center sm:justify-start space-x-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-12 py-4 rounded-xl bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 focus:border-amber-500 dark:focus:border-amber-400 focus:ring-2 focus:ring-amber-500/20 text-slate-900 dark:text-slate-100 placeholder-slate-400 dark:placeholder-slate-500 text-base font-medium transition-all"
              />
              {searchQuery && (
                <button
                  onClick={() => setSearchQuery('')}
                  className="absolute inset-y-0 right-0 pr-4 flex items-center text-slate-400 hover:text-slate-700 dark:hover:text-slate-200"
                >
                  <X className="w-5 h-5" />
                </button>
              )}
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 pt-1">
              <div className="flex flex-wrap items-center gap-1.5 text-xs">
                <span className="text-slate-600 dark:text-slate-400 font-semibold flex items-center space-x-1 mr-1">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg font-bold transition-all ${
                      categoryFilter === c.id
                        ...
                        : 'bg-slate-100 dark:bg-slate-900 text-slate-700 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 border border-slate-200 dark:border-slate-800'
                    }`}
                  >
                    {c.label}
                  </button>
            ...
            {highlightedMatch && (
              <div className="p-6 rounded-2xl bg-gradient-to-br from-amber-50 via-white to-amber-100/50 dark:from-amber-500/20 dark:via-slate-900 dark:to-amber-950/30 border-2 border-amber-300 dark:border-amber-500/40 shadow-xl space-y-4 animate-slide-up">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-extrabold uppercase tracking-widest px-3 py-1 rounded-full bg-amber-100 dark:bg-amber-500/20 text-amber-800 dark:text-amber-300 border border-amber-300 dark:border-amber-500/30">
                    ⚡ Coincidencia Destacada
                  </span>
                  <span className="text-xs font-mono text-slate-600 dark:text-slate-400">
                    Traducción: <strong>{highlightedMatch.translation}</strong>
                  </span>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center">
                  ...
                  {/* Base form */}
                  <div className="p-3.5 rounded-xl bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 text-center shadow-sm">
                    <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 block">Infinitivo / Base</span>
                    <span className="text-xl sm:text-2xl font-black text-slate-900 dark:text-slate-100 font-mono">{highlightedMatch.base}</span>
                  </div>

                  {/* Past simple form (PROMINENT!) */}
                  <div className="p-4 rounded-xl bg-amber-500 dark:bg-amber-500/20 border-2 border-amber-400 text-center shadow-lg transform sm:scale-105">
                    <span className="text-[10px] uppercase font-extrabold text-amber-950 dark:text-amber-300 block tracking-wider">
                      PASADO SIMPLE (PAST)
                    </span>
                    <span className="text-2xl sm:text-3xl font-black text-white dark:text-amber-200 font-mono">
                  ...
                  {/* Participle */}
                  <div className="p-3.5 rounded-xl bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 text-center shadow-sm">
                    <span className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 block">Participio Pasado</span>
                    <span className="text-xl sm:text-2xl font-black text-slate-800 dark:text-slate-300 font-mono">{highlightedMatch.participle}</span>
                  </div>

                </div>

                {/* Example sentence */}
                <div className="p-3 rounded-xl bg-white/80 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 text-xs sm:text-sm space-y-1">
                  <span className="text-slate-600 dark:text-slate-400 font-semibold block">Ejemplo en contexto IT / Programación:</span>
                  <p className="text-slate-900 dark:text-amber-100 font-medium italic">
          ...
          <div className="glass-card rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-xl">
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              ...
              <span className="text-xs text-slate-500 dark:text-slate-400">Total: {filteredVerbs.length} registros</span>
            </div>

            <div className="overflow-x-auto max-h-[500px] overflow-y-auto">
              <table className="w-full text-left text-xs sm:text-sm">
                <thead className="sticky top-0 bg-slate-50 dark:bg-slate-950/95 border-b border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 uppercase text-[10px] tracking-wider z-10">
                  <tr>
                    <th className="py-3 px-4">Infinitivo (Base)</th>
                    <th className="py-3 px-4 text-amber-700 dark:text-amber-400 font-bold">Pasado Simple</th>
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60 font-mono">
                  {filteredVerbs.map((v, idx) => (
                    ...
                    >
                      <td className="py-3 px-4 font-bold text-slate-900 dark:text-slate-100">{v.base}</td>
                      <td className="py-3 px-4 font-extrabold text-amber-800 dark:text-amber-300 text-sm sm:text-base bg-amber-500/10 group-hover:bg-amber-500/20">
                        {v.past}
                      </td>
                      <td className="py-3 px-4 text-slate-700 dark:text-slate-300">{v.participle}</td>
                      <td className="py-3 px-4 font-sans text-slate-700 dark:text-slate-300">{v.translation}</td>
                      <td className="py-3 px-4 font-sans text-slate-500 dark:text-slate-400 text-xs hidden md:table-cell italic">
                        {v.example}
                      </td>
                    </tr>
                  ))}
          ...
          <div className="glass-card rounded-2xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base sm:text-lg flex items-center space-x-2">
            ...
            <div 
              ...
              className="min-h-[180px] p-8 rounded-2xl bg-gradient-to-br from-slate-50 to-amber-50/40 dark:from-slate-900 dark:to-slate-950 border-2 border-slate-200 dark:border-slate-800 hover:border-amber-400 dark:hover:border-amber-500/40 transition-all duration-300 cursor-pointer flex flex-col items-center justify-center text-center space-y-3 shadow-md"
            >
              {!isFlipped ? (
                <>
                  ...
                  <div className="text-3xl sm:text-4xl font-black text-slate-900 dark:text-slate-100 font-mono">
                ...
                <div className="space-y-2 animate-fade-in">
                  ...
                  <div className="text-4xl sm:text-5xl font-black text-amber-800 dark:text-amber-300 font-mono">
                    {currentFlashcard.past}
                  </div>
                  <p className="text-xs text-slate-600 dark:text-slate-300 max-w-md pt-2 italic">
            ...
            <div className="flex items-center justify-between pt-2">
              <button
                ...
                className="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 text-xs font-bold border border-slate-200 dark:border-slate-800"
              >
                Anterior
              </button>
              <button
                ...
                className="px-4 py-2 rounded-xl bg-amber-100 dark:bg-amber-500/20 text-amber-800 dark:text-amber-300 text-xs font-bold border border-amber-300 dark:border-amber-500/30"
              >
          ...
          <div className="glass-card rounded-2xl p-6 border border-slate-200 dark:border-slate-800 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
              {data.regularVsIrregular.regular.spelling.map((sp, idx) => (
                <div key={idx} className="p-3.5 rounded-xl bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-xs space-y-1">
                  <span className="font-bold text-slate-800 dark:text-slate-200 block">{sp.rule}</span>
                  <div className="text-emerald-700 dark:text-emerald-300 font-mono font-medium">{sp.examples.join(' · ')}</div>
                </div>
              ))}
            </div>
          </div>
                ...
                <div key={idx} className="glass-card rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden shadow-md">
                  ...
                  {isOpen && (
                    <div className="px-5 pb-6 pt-2 space-y-5 border-t border-slate-200 dark:border-slate-800/80 animate-fade-in">
                      ...
                      {/* Formula */}
                      <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 font-mono text-xs sm:text-sm text-amber-800 dark:text-amber-300">
                      ...
                      <div className="space-y-2 text-sm text-slate-700 dark:text-slate-300">
                        <span className="text-xs font-bold uppercase tracking-wider text-slate-600 dark:text-slate-400 block">Reglas Clave:</span>
      ...
      {activeTab === 'readings' && (
        <div className="space-y-6 animate-fade-in">
          {data.readingCaseStudies.map((study, idx) => (
            <div key={idx} className="glass-card rounded-2xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 space-y-4">
              ...
              <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                {study.tags.map((tag, tIdx) => (
                  <span key={tIdx} className="text-xs px-2.5 py-1 rounded-md bg-slate-100 dark:bg-slate-900 text-cyan-800 dark:text-cyan-300 border border-slate-200 dark:border-slate-800 font-mono">
                    #{tag}
                  </span>
                ))}
              </div>
      ...
      {activeTab === 'glossary' && (
        <div className="glass-card rounded-2xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 space-y-6 animate-fade-in">
          <div className="space-y-1">
            <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center space-x-2">
              <Building2 className="w-5 h-5 text-cyan-600 dark:text-cyanBrand-400" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {data.businessGlossary.map((item, idx) => (
              <div key={idx} className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800 space-y-2 hover:border-slate-300 dark:hover:border-slate-700 transition-colors">
                <span className="font-bold text-amber-800 dark:text-amber-300 text-sm block font-mono">
                  {item.term}
                </span>
        ...
        <div className="glass-card rounded-2xl p-6 sm:p-8 border border-slate-200 dark:border-slate-800 space-y-6 animate-fade-in">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            <button
              ...
              className="text-xs text-amber-700 dark:text-amber-400 hover:text-amber-800 dark:hover:text-amber-300 font-semibold underline self-start sm:self-auto"
            >
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    {q.options.map((opt, optIdx) => {
                      ...
                      if (quizChecked) {
                        ...
                        btnStyle = 'bg-amber-50 dark:bg-amber-600/30 border-amber-500 dark:border-amber-400 text-amber-900 dark:text-white font-bold ring-2 ring-amber-500/30';
                      }
